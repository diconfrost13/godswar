...
map< string, string >  RegisterService::m_valid_map;		//校验码
...
void PipeService::Release()
{
	...
	delete this;
	//   m_pWorld->g_pBackUpMgr->Finalize();
}
...
void PipeService::Run(  )
{
	...
	//if ( m_CheckTimer.IsExpire() )
	//{
	//	m_CheckTimer.Startup(CHECKT_PIPE_TIMER);

	//	if ( !m_pWorld->g_pBackUpMgr->CheckPipeState() )
	//	{
	//		sbase::ConsoleWriteColorText(FOREGROUND_RED, "IPC_BACKUP PipeServer Close!");
	//		return;
	//	}
...
void  CommandService::Key_Reset( char *, pFunPrintf pPrintf )         
{
	...
	for( int i=0, j=1; i<m_pWorld->g_lMainCity; i++, j++ )				// Athens/Sparta
	{
...
void  CommandService::Key_Role( char * command, pFunPrintf pPrintf )          
{
	...
	sprintf( pro, "%s",pPlayer->GetlClass());//@@@ 职业名字
...
void  CommandService::Key_Queue( char * , pFunPrintf pPrintf )        
{
	//20080618
	//  	pPrintf( "   ...[Execute]\n");
	//  	pPrintf("Current message number in IN queue is: %d\n", g_pLoader->GetNet2LogicQueueCount());
	//  	pPrintf("Current message number in OUT queue is: %d\n", g_pLoader->GetLogic2NetQueueCount());
	//  	pPrintf( "GodsWar> " );
}
		...
		case 8:				// BackSpace
			{
				...
				{
					printf( "\b \b" );
					//sbase::ConsoleWriteColorText( FOREGROUND_BLUE, "\b \b" );
		...
		case 27:			// ESC
			{
				lKeyCount = 0;
				memset( strKeyBuffer, 0, 256 );
				//sbase::ConsoleWriteColorText( FOREGROUND_BLUE, "   ...[Cancel]\nCommand: " );
				printf("   ...[Cancel]\nCommand: ");
			}
			break;
		default:
			{
				...
				lKeyCount++;
				//sbase::ConsoleWriteColorText( FOREGROUND_BLUE, "%c", key );
				printf("%c", key );
			}
			break;
		}
...
// bool RegisterService::PackMsg( char* pMsg, int nLen )
// {
// 	if ( NULL != m_pClientSck)
// 		m_pClientSck->PackMsg( pMsg, nLen );
// 	return true;
// }
...
const SSockHandle*   ServerSocketEventHandle::GetSockHandleTab( USHORT &Size ) const
{
	static const SSockHandle  MsgEventHandle[] = 
	{
		...
		{ _MSG_CREATE_ROLE,             		&ServerSocketEventHandle::Handle_CreateRole   		},//Fenjune
		{ _MSG_DELETE_ROLE,             		&ServerSocketEventHandle::Handle_DelRole      		},//Fenjune
		{ _MSG_WALK_BEGIN,              		&ServerSocketEventHandle::Handle_WalkBegin    		},//Fenjune
		{ _MSG_WALK,              	          	&ServerSocketEventHandle::Handle_Walk    		    },//Fenjune
		{ _MSG_WALK_END,                		&ServerSocketEventHandle::Handle_WaklEnd      		},//Fenjune
		{ _MSG_FIGHT,                   		&ServerSocketEventHandle::Handle_Fight        		},//Fenjune
		{ _MSG_ATTACK,                  		&ServerSocketEventHandle::Handle_Attack       		},//Fenjune
		{ _MSG_SKILL,                   		&ServerSocketEventHandle::Handle_Skill        		},//zwx
		{ _MSG_LEARN,                   		&ServerSocketEventHandle::Handle_Learn        		},//zwx
		{ _MSG_SKILLBACKUP,             		&ServerSocketEventHandle::Handle_SkillBackup  		},//zwx
		{ _MSG_SKILL_UPGRADE,           		&ServerSocketEventHandle::Handle_SkillUpgrade 		},//Fenjune
		{ _MSG_SKILL_INTERRUPT,           		&ServerSocketEventHandle::Handle_SkillInterrupt 	},//zwx
		{ _MSG_EFFECT,                  		&ServerSocketEventHandle::Handle_Effect       		},//Fenjune
		{ _MSG_TALK,                    		&ServerSocketEventHandle::Handle_Talk         		},//C.D
		{ _MSG_TALKCHANNEL,						&ServerSocketEventHandle::Handle_TalkChannel		},//C.D
		{ _MSG_PICKUPDROPS,             		&ServerSocketEventHandle::Handle_Drop         		},//Fenjune
		{ _MSG_USEOREQUIP,              		&ServerSocketEventHandle::Handle_Equip        		},//tao
		{ _MSG_MOVEITEM,                		&ServerSocketEventHandle::Handle_MoveItem     		},//tao
		{ _MSG_BREAK_ITEM,                		&ServerSocketEventHandle::Handle_BreakItem     		},//twy
		{ _MSG_SELL,                    		&ServerSocketEventHandle::Handle_Sell         		},//tao
		{ _MSG_STORAGEITEM,             		&ServerSocketEventHandle::Handle_StorageItem  		},//tao
		{ _MSG_STALL,                   		&ServerSocketEventHandle::Handle_Stall        		},//C.D
		{ _MSG_STALLITEM,               		&ServerSocketEventHandle::Handle_StallItem    		},//C.D
		{ _MSG_STALLADDITEM,            		&ServerSocketEventHandle::Handle_StallAddItem 		},//C.D
		{ _MSG_STALLDELITEM,            		&ServerSocketEventHandle::Handle_StallDelItem 		},//C.D
		{ _MSG_STALLBUYITEM,            		&ServerSocketEventHandle::Handle_StallBuyItem 		},//C.D
		{ _MSG_TALKNPC,                 		&ServerSocketEventHandle::Handle_TalkNPC      		},//zwx
		{ _MSG_NPCDATA,                 		&ServerSocketEventHandle::Handle_NPCData      		},//cdx_ok
		{ _MSG_SYS_NPC_DATA,                    &ServerSocketEventHandle::Handle_SysFunNPC          },//Fenjune
		{ _MSG_NPCSELL,                 		&ServerSocketEventHandle::Handle_NPCSell      		},//cdx_ok
		{ _MSG_MALLSELL,                 		&ServerSocketEventHandle::Handle_MallSell			},//cdx_ok
		{ _MSG_ASSOCIATIONSELL,                 &ServerSocketEventHandle::Handle_AssociationSell    },//cdx_ok
		{ _MSG_MALLITEMDATA,					&ServerSocketEventHandle::Handle_MallItemData		},//cdx_ok
		{ _MSG_ASSOCIATIONITEMDATA,				&ServerSocketEventHandle::Handle_AssociationItemData},//cdx_ok
		{ _MSG_FINDQUEST,               		&ServerSocketEventHandle::Handle_FindQuest    		},//zwx
		{ _MSG_NPCACCEPTQUEST,          		&ServerSocketEventHandle::Handle_NPCAcceptQuest     },//zwx
		{ _MSG_NPCCANCELQUEST,          		&ServerSocketEventHandle::Handle_NPCCancelQuest     },//zwx
		{ _MSG_NPCCOMPLETEQUEST,        		&ServerSocketEventHandle::Handle_NPCCompleteQuest   },//zwx
		{ _MSG_NPCQUESTVIEW,            		&ServerSocketEventHandle::Handle_NPCQuestView       },//zwx
		{ _MSG_RELATION_REQUEST,        		&ServerSocketEventHandle::Handle_RelationRequest    },//Fenjune
		{ _MSG_RELATION_RESPONSE,       		&ServerSocketEventHandle::Handle_RelationResponse   },//Fenjune
		{ _MSG_RELATION_DELETE,         		&ServerSocketEventHandle::Handle_RelationDel        },//Fenjune
		{ _MSG_TRADE,                   		&ServerSocketEventHandle::Handle_Trade              },//cdx_ok
		{ _MSG_TRADE_MONEY,             		&ServerSocketEventHandle::Handle_TradeMoney         },//cdx_ok
		{ _MSG_TRADE_ADDITEM,           		&ServerSocketEventHandle::Handle_TradeAddItem       },//cdx_ok
		{ _MSG_TRADE_REMOVEITEM,        		&ServerSocketEventHandle::Handle_TradeRemoveItem    },//cdx_ok
		{ _MSG_EQUIPFORGE,              		&ServerSocketEventHandle::Handle_EquipForge         },//tao
		{ _MSG_EQUIPFORGE_EQUIP,        		&ServerSocketEventHandle::Handle_EquipForge_Equip   },//tao
		{ _MSG_EQUIPFORGE_MATERIAL,     		&ServerSocketEventHandle::Handle_EquipMaterial      },//tao
		{ _MSG_EQUIPFORGE_EQUIPCANCEL,  		&ServerSocketEventHandle::Handle_EquipCancel        },//tao
		{ _MSG_EQUIPFORGE_MATERIALCANCEL,       &ServerSocketEventHandle::Handle_EquipMaterialCancel},//tao
		{ _MSG_EQUIPFORGE_CANCEL,           	&ServerSocketEventHandle::Handle_EquipForgeCacel    },//tao
		{ _MSG_TEAM_INVITE,                 	&ServerSocketEventHandle::Handle_TeamInvite         },//C.D
		{ _MSG_TEAM_REJECT,                 	&ServerSocketEventHandle::Handle_TeamReject         },//C.D
		{ _MSG_TEAM_INFO,						&ServerSocketEventHandle::Handle_TeamInfo			},//C.D
		{ _MSG_TEAM_ADD,                    	&ServerSocketEventHandle::Handle_TeamAdd            },//C.D
		{ _MSG_TEAM_DELETE,                 	&ServerSocketEventHandle::Handle_TeamDel            },//C.D
		{ _MSG_TEAM_REPLACELEADER,          	&ServerSocketEventHandle::Handle_TeamLeader         },//C.D
		{ _MSG_TEAM_DISSOLVE,               	&ServerSocketEventHandle::Handle_TeamDissolve       },//C.D
		{ _MSG_TEAM_LEAVE,                  	&ServerSocketEventHandle::Handle_TeamLeave          },//C.D
		{ _MSG_BACKHOME,                    	&ServerSocketEventHandle::Handle_BackHome           },//cdx
		{ _MSG_CONSORTIA_CREATE,            	&ServerSocketEventHandle::Handle_ConsortiaCreate    },//cdx
		{ _MSG_CONSORTIA_INVITE,            	&ServerSocketEventHandle::Handle_ConsortiaInvite    },//cdx
		{ _MSG_CONSORTIA_DISMISS,           	&ServerSocketEventHandle::Handle_ConsortiaDimiss    },//cdx
		{ _MSG_CONSORTIA_RESPONSE,          	&ServerSocketEventHandle::Handle_ConsortiaResponse  },//cdx
		{ _MSG_CONSORTIA_EXIT,              	&ServerSocketEventHandle::Handle_ConsortiaExit      },//cdx
		{ _MSG_CONSORTIA_TEXT,              	&ServerSocketEventHandle::Handle_ConsortiaText      },//cdx
		{ _MSG_CONSORTIA_DUTY,              	&ServerSocketEventHandle::Handle_ConsortiaDuty      },//cdx
		{ _MSG_CONSORTIA_MEMBER_DEL,        	&ServerSocketEventHandle::Handle_ConsortiaMemDel    },//cdx
		{ _MSG_CONSORTIA_LVUP,					&ServerSocketEventHandle::Handle_ConsortiaLevelUp   },//cdx
		{ _MSG_ALTAR_CREATE,					&ServerSocketEventHandle::Handle_CreateAltar        },//cdx
		{ _MSG_ALTAR_LVUP,						&ServerSocketEventHandle::Handle_AltarLevelUp       },//cdx
		{ _MSG_ALTAR_OBLATION,					&ServerSocketEventHandle::Handle_AltarOblation      },//cdx
		{ _MSG_CRETIT_EXCHANGE,					&ServerSocketEventHandle::Handle_CretitExchange     },//cdx
		{ _MSG_GOLD_BOX,					    &ServerSocketEventHandle::Handle_OpenGoldBox        },//lion 宝箱系统
		{ _MSG_EXPLORER_QUEST,               	&ServerSocketEventHandle::Handle_Explorer    		},//zwx探索验证
		{ _MSG_TARGETINFO,		              	&ServerSocketEventHandle::Handle_GetTargetInfo	    },//cdx_ok
		{ _MSG_DELAY_EXIT,                      &ServerSocketEventHandle::Handle_DelayExit          },//cdx_ok
	};
...
{
	// 	snet::CSocket  *pSocket = static_cast<net::CSocket*>(pPortMsg->GetTo());
	// 	CPlayer::ClearValidCode( pSocket );
	// 	CPlayer* pPlayer = GetPlayerFromSocket( pPortMsg->GetTo() );
	// 	if( pPlayer == NULL )
	// 	{
	// 		sbase::ConsoleWriteColorText( FOREGROUND_RED,"_KERNEL_MSG_CONNECT_CLOSE" );
	// 		return NO_MSG_ERRO;
	// 	}
	// 	//是否在开启宝箱
	...
	// 		CDial::Instance()->DefaultItem(pPlayer->GetGoldBoxId());
	// 		if(pPlayer->IsDoubleItem())
	// 		{
	// 			CDial::Instance()->SecondItem(pPlayer->GetGoldBoxId());
	// 		}
	// 
	// 	}
	// 
	// 
	// 	if ( pPlayer->GetJoinFlag() )
	// 	{
	// 		if (pPlayer->IsDead())
	// 		{
	// 			pPlayer->Revive();
	// 
	// 			for ( list< tarSceneChange* >::iterator iter = pPlayer->s_World->g_listSceneChange.begin() ; iter != pPlayer->s_World->g_listSceneChange.end() ; iter++)
	// 			{
	// 				if ((*iter)->pPlayer == pPlayer)
	// 				{
	// 					GetRegion( (*iter)->lFromSceneID )->RemoveObject( (*iter)->pPlayer->GetID() );
	// 					(*iter)->pPlayer->SetPos( (*iter)->x, 0.0f, (*iter)->z );
	// 					(*iter)->pPlayer->SetlRegionID(  GetRegion((*iter)->lToSceneID)->GetLogicID() ) ;
	// 
	// 					SAFE_DELETE(*iter);
	// 
	// 					pPlayer->s_World->g_listSceneChange.erase(iter);
	// 
	// 					break;
	// 				}
	// 			}
	// 		}
	// 
	// 		pPlayer->GetRegion()->RemovePlayer( pPlayer );
	// 
	// 	}
	// 
	// 
	// 	//防止重复登陆后造成先登陆的玩家帐号清掉
	// 	if ( pPlayer ==  GetPlayerFromAccounts( pPlayer->GetAccounts() ))
	// 	{
	// 		CPlayer::DelAccountsHashMap( pPlayer->GetAccounts() );
	// 	}
	// 
	// 	CPlayer::DelRoleNameHasnMap( pPlayer->GetName() );	
	// 	CPlayer::DelSocketHashMap( pSocket );
	// 
	// 	//如果该玩家在组队状态，则自动脱离队伍
	// 	pPlayer->LeaveTeam_New();
	// 	RemovePlayer( pPlayer );
	// 
	// 	sbase::CSingleLock xLock1(&m_xLock);
	// 	m_pPort->ClearSockMsg( (void*)pSocket );
...
eError ServerSocketEventHandle::Handle_ClientReady( const void* pBuf, CPlayer* pPlayer )
{
	...
	/*
	wchar_t w_Hint[256];
	sbase::CIni  HintIni( "config.ini", false );
	const char *Hint = HintIni.GetString( "Note", "Hint");
	int counter = MultiByteToWideChar( CP_ACP, 0, Hint, -1, 0, 0 );
	MultiByteToWideChar( CP_ACP, 0, Hint, -1, w_Hint, counter ); 
	pPlayer->GetRegion()->Talk( pPlayer, pPlayer, w_Hint, (int)(::wcslen( w_Hint )+1)*2, defSYSTEM,0 );
	*/
	return NO_MSG_ERRO;
}
...
eError ServerSocketEventHandle::Handle_LoginGS( const void* pBuf, CPlayer* pPlayer )
{
	...
	if (  LoginedPlayer != NULL )
	{
		...
		LoginedPlayer->DelayExit(0);//立即退出 
		Sleep(1);
...

	{
		...
		{ 
			...
			//排队开始
			//pPlayer->s_World->StartQueue(pPlayer);
		}
		else
		{
			sbase::ConsoleWriteColorText( FOREGROUND_RED, "_MSG_LOGIN_GAMESERVER" );
...
eError ServerSocketEventHandle::Handle_CreateRole( const void* pBuf, CPlayer* pPlayer )
{
	...
	if ( CKeyWords::Instance()->FindKeyWord( pCreateRole->Name ))
	{
		...
		error.ucDisc = MSG_ERRO_0003;           //包含非法字符
		pPlayer->s_World->SendMsgToClient( &error, pPlayer->GetSocket() );
		return NO_MSG_ERRO;
	}
	...
	if ( pPlayer->GetHaveRoleNum() >= 1 )
	{
		...
		error.Head.usType = _MSG_MANAGE_RETURN;
		error.ucErrorType = _MSG_CREATE_ROLE ;
		error.ucDisc = MSG_ERRO_0004;           //角色个数限制
		pPlayer->s_World->SendMsgToClient( &error, pPlayer->GetSocket() );
		return NO_MSG_ERRO;
	}
...
eError ServerSocketEventHandle::Handle_WalkBegin(const void* pBuf, CPlayer* pPlayer )
{
	...
	{
		//	pPlayer->AddDanger();
		//	sbase::LogException("Message type Exception ,Accounts : %s, Role: %s,[_MSG_WALK_BEGIN]", pPlayer->GetAccounts(), pPlayer->GetName() );
		return NO_MSG_ERRO;
	}
	...
	if( pPlayer->GetSocket() == NULL )			// 验证socket和id的合法性
	{
...
eError ServerSocketEventHandle::Handle_WaklEnd(const void* pBuf, CPlayer* pPlayer )
{
	...
	{
		//pPlayer->AddDanger();
		//sbase::LogException("Message type Exception ,Accounts : %s, Role: %s,[_MSG_MSG_WALK_END]",pPlayer->GetAccounts(), pPlayer->GetName() );
		return NO_MSG_ERRO;
	}
...
eError  ServerSocketEventHandle::Handle_Skill( const void* pBuf, CPlayer* pPlayer )
{
	...
	if ( pPlayer->GetSocket() != NULL )
	{
		...
		//if( pTarget == NULL )
		//{
		//	sbase::ConsoleWriteColorText( FOREGROUND_RED,"_MSG_SKILL pTarget" );
		//	return;
		//}
...
eError ServerSocketEventHandle::Handle_SkillUpgrade( const void* pBuf, CPlayer* pPlayer )
{
	...
	return NO_MSG_ERRO;
	//SendErrorInfo(_MSG_SKILL_UPGRADE,ErrorId,pPlayer->GetSocket());
}
...
eError ServerSocketEventHandle::Handle_MoveItem( const void* pBuf, CPlayer* pPlayer )
{
	...
	ErrorId = pPlayer->MoveItem(pMsg->Index , pMsg->Num , pMsg->IndexAim , pMsg->NumAim/*,-1*/);//改类型
...
eError	ServerSocketEventHandle::Handle_StallAddItem( const void* pBuf, CPlayer* pPlayer )
{
	...
	if ( pMsg->Index<0 || pMsg->Index>=MAX_GOODS || pMsg->Pocket<0 || pMsg->Pocket>=MAX_POCKETS
		|| pMsg->BagIndex > pPlayer->GetuiBagNum()/*注意！*/ || pMsg->BagIndex >= MAX_BAGS || pMsg->BagIndex<0 || pMsg->BagNum<0 || pMsg->BagNum>=MAX_BAG_GRID
...
eError	ServerSocketEventHandle::Handle_TalkNPC( const void* pBuf, CPlayer* pPlayer )
{
	...
	if ( pPlayer->GetlFaction() != pNpc->GetlFaction() && pNpc->GetlFaction() != 2 )//阵营2为友好阵营
	{
...
eError	ServerSocketEventHandle::Handle_RelationRequest( const void* pBuf, CPlayer* pPlayer )
{
	...
	if( pRelationPlayer == NULL  )  //好友才判断在线
	{
		if ( Result == -1 )  //找不到该角色信息
		{ 
			// 			//不能正确添加添加该好友
			// 			MSG_RELATION_REQUEST  Msg;
			// 			Msg.Head.usSize = sizeof(MSG_RELATION_REQUEST);
			// 			Msg.Head.usType = _MSG_RELATION_REQUEST;
			// 			Msg.cType   =  pMsg->cType;
			// 			Msg.Error       = Result;
			// 			memcpy( Msg.cName_Request,pPlayer->GetName(),sizeof(Msg.cName_Request) );
			// 			memset( Msg.cName_Response,0,32);
			// 			memcpy( Msg.cName_Response,pMsg->cName_Response,sizeof(Msg.cName_Response));
			// 			SendMsgToClient( &Msg, pPlayer->GetSocket() );
			return MSG_ERRO_021E;
		}
	}

	//判断好友列表并且更新(是否添加过该好友，好友列表是否已满)
	...
	if ( !error )
	{
		if( !pMsg->cType  ) 
		{
			if ( pPlayer->GetlFaction() ==  pRelationPlayer->GetlFaction()  ) //相同阵营
			{
...
eError	ServerSocketEventHandle::Handle_EquipForge( const void* pBuf, CPlayer* pPlayer )
{
	...
	sbase::ConsoleWriteColorText( FOREGROUND_RED,"recv _MSG_EQUIPFORGE" );
...
eError	ServerSocketEventHandle::Handle_EquipMaterial( const void* pBuf, CPlayer* pPlayer )
{
	...
	sbase::ConsoleWriteColorText( FOREGROUND_RED,Ifbuf );//tao
...
eError	ServerSocketEventHandle::Handle_TeamInvite( const void* pBuf, CPlayer* pPlayer )
{
	...
	if ( !pAimPlayer)//双方有一方不存在
	{
		sbase::ConsoleWriteColorText( FOREGROUND_RED,"_MSG_TEAM_INVITE" );
		//消息返回resPlayer，该玩家邀请失败
		MSG_TIP msg_tip;
	...
	if ( NULL == pAimPlayer->m_pTeamLeader )
	{
		...
		//情况2：目标玩家没有组队，自己组队但不是队长
		if ( (NULL != pResPlayer->m_pTeamLeader)&&
			(!pResPlayer->IsLeader()) )
		{
			//提示，resPlayer玩家，没有权限（该步在客户端也进行一次判断）

			return NO_MSG_ERRO;
		}

		//情况3：目标玩家没有组队，自己组队并且是队长
		if ( (NULL != pResPlayer->m_pTeamLeader)&&
			(pResPlayer->IsLeader()) )
		{
			//做法同情况1
	...
	else
	{
		...
		{	//自己没有队伍，但是对方有队伍，则转交于对方队长，提出申请，要求加入队伍
...
eError	ServerSocketEventHandle::Handle_TeamAdd( const void* pBuf, CPlayer* pPlayer )
{
	...
	if ( pResPlayer )
	{
		ErrorId = pResPlayer->AddTeammate_New( pAimPlayer );//改类型
	}
	else
	{
...
eError	ServerSocketEventHandle::Handle_ConsortiaCreate( const void* pBuf, CPlayer* pPlayer )
{
	...
	if(pPlayer)
	{
		...
		else
		{
			...
			NewConsortia.uiID = -1;
			NewConsortia.lfound = Basefound->lfound;                       //资金
...
eError	ServerSocketEventHandle::Handle_ConsortiaInvite( const void* pBuf, CPlayer* pPlayer )
{
	...
	{
		if ( pInvitedPlayer->HaveConsortia() )               //被邀请者有公会 
		{
			return MSG_ERRO_0368;
		}
		else if ( pInvitedPlayer->GetlFaction() != pInvitePlayer->GetlFaction() )  //不同阵营不能邀请
		{
			return MSG_ERRO_0369;
		}
		else if ( pInvitePlayer->GetcUnionBusiness() < CONSORTIA_DIRECTOR )  //职位小于理事将不能邀请
		{
			return MSG_ERRO_036A;
		}
		else if ( pInvitePlayer->IsMemeberFull() )           //所在公会是否满员
		{
			return MSG_ERRO_036B;
		}
		else if ( pInvitePlayer->s_World->g_pConsortiaManager->HaveMember( pInvitePlayer->GetConsortia(), pInvitedPlayer->GetName() ) ) //被邀请者已经是会员了
		{
...
eError	ServerSocketEventHandle::Handle_ConsortiaResponse( const void* pBuf, CPlayer* pPlayer )
{
	...
	if( pReceivePlayer && pSendPlayer && !pSendPlayer->HaveConsortia() ) //防止多个人邀请一个人入会
	{
		if( 0 == msg->Style && pReceivePlayer->GetConsortia()) //防止会长邀请玩家入会，在玩家点击确定时会长把公会删除掉
		{
			if ( pReceivePlayer->s_World->g_pConsortiaManager->HaveMember( pReceivePlayer->GetConsortia(), pSendPlayer->GetName() ) ) //被邀请者已经是会员了
				return NO_MSG_ERRO;
			if ( pReceivePlayer->s_World->g_pConsortiaManager->IsMemberFull( pReceivePlayer->GetConsortia() ))  //公会满员
				return MSG_ERRO_036B;
...
eError	ServerSocketEventHandle::Handle_ConsortiaExit( const void* pBuf, CPlayer* pPlayer )
{
	...
	//SendErrorInfo(_MSG_CONSORTIA_EXIT,0,pPlayer->GetSocket());
	return NO_MSG_ERRO;
}
...
eError  ServerSocketEventHandle::Handle_ConsortiaLevelUp(const void* pBuf, CPlayer* pPlayer)
{
	...
	if ( NULL != pPlayer )
	{
		return pPlayer->ConsortiaLevelup();//改类型
	}
...
eError ServerSocketEventHandle::Handle_CreateAltar( const void* pBuf, CPlayer* pPlayer )
{
	...
	if ( NULL != pPlayer )
	{
		pPlayer->CreateAltar(pAltarMsg->iAltarID);//改类型
	}
...
eError ServerSocketEventHandle::Handle_AltarLevelUp( const void* pBuf, CPlayer* pPlayer )
{
	...
	if ( NULL != pPlayer )
	{
		...
		pPlayer->AltarLevelUp(AltarMsg->iAltarID);//改类型
...
eError ServerSocketEventHandle::Handle_OpenGoldBox( const void* pBuf, CPlayer* pPlayer )
{
	...
	if(CDial::Instance()->isMsgWrong(pOpenGoldBox)) //消息范围检测
	{
		return NO_MSG_ERRO;
	}
		...
		case 8://临时方法
			{
				...
				//			if(!pPlayer->IsGoldBoxExist(pOpenGoldBox->GoldBoxId)){return;}//没有宝箱
		...
		case 0://准备打开转盘
			{
				...
				pPlayer->SetCurrentIter(pOpenGoldBox->Iter);//记录宝箱位置
				pPlayer->SetGoldBoxId(pOpenGoldBox->GoldBoxId);//记录宝箱ID
...
eError	ServerSocketEventHandle::Handle_GetTargetInfo(const void* pBuf, CPlayer* pPlayer )
{
	...
	//	g_pLoader->SendMsgToClient( pMsg , pResPlayer->GetSocket());
