...
void CNPC::LoadSysGift()
{
	...
	// 
	// 	PyObject *pDict = NULL;
	// 	pDict = PyObject_GetAttrString(m_Module , "GetItemList");
	// 
	// 	if (!pDict)
	// 	{
	// 		PyErr_Clear();
	...
	// 	}
	// 
	// 	if ( PyDict_Check(pDict) == 1 )
	// 	{
	// 		PyObject* pKeys = PyDict_Keys(pDict);
	// 		for(int i = 0; i < PyDict_Size(pDict); ++i)
	// 		{
...
void CNPC::LoadQuest()
{
	...
	//ASSERT(tuplesize <= QUEST_NPCTAKE_COUNT);
	...
	for (int i = 0 ; i < tuplesize ; i++)
	{
		...
		//printf("NPC:%s Quest:%d%\t", GetName(), PyInt_AsLong(QuestID) );
		...
		//PyObject* Color = PyDict_GetItemString(obj->cl_dict,"Color");
		...
		PyObject* TeamLimit = PyDict_GetItemString( obj->cl_dict, "TeamLimit");//队伍限制

		PyObject* RewardUionBijou = PyDict_GetItemString( obj->cl_dict, "RewardUionBijou");//公会任务奖励金
		PyObject* RewardUionMoney = PyDict_GetItemString( obj->cl_dict, "RewardUionMoney");//公会任务奖励银
		PyObject* ReqConsortiaLV = PyDict_GetItemString( obj->cl_dict, "ReqConsortiaLV");//公会任务级别限制
		...
		m_Quests[i].TeamLimit = PyInt_AsLong(TeamLimit);//队伍限制
		...
		//m_Quests[i].Color = PyInt_AsLong(Color);
...
void CNPC::GetItemList(CPlayer* player)
{
	...
	// 
	// 	for (int i = 0 ; i < ACADEMIC_NPC_MEM_ID_ITEMTABS ; i++)
	// 	{
	// 		msg_npc_itemdata.ItemTabs[i] = 0;
	// 		msg_npc_itemdata.ItemTabsName[i] = 0;
	// 	}
	// 
	// 	for (size_t i = 0 ; i < m_Items.size() ; i++)
	// 	{
	// 		msg_npc_itemdata.ItemTabs[i] = (char)m_Items[i].size();
	// 		msg_npc_itemdata.ItemTabsName[i] = m_NpcSell[i];
	// 	}
	// 	msg_npc_itemdata.ItemCount = (int)m_ItemBases.size();
	// 
	// 	ASSERT(ACADEMIC_NPC_MEM_ID_ITEMS > msg_npc_itemdata.ItemCount);
	// 
	// 	if (!m_ItemBases.empty())
	// 	{
	...
	// 
	// 	g_pLoader->SendMsgToClient(&msg_npc_itemdata , player->GetSocket());


	////////////////
	//对数据包进行整合， 每个包裹发送的是实际的数据大小 by_cdx
	ASSERT((int)m_NpcSell.size() == (int)m_Items.size());
...
{
	//msg_npc_storagedata.NPCID = this->GetID();
	//msg_npc_storagedata.Storage = player->GetStorage()->GetActive();
	player->SendStorageMsg();
	//g_pLoader->SendMsgToClient(&msg_npc_storagedata , player->GetSocket());
...
{
	//如果发现客户端资源不对，会同步贩卖列表，大部分因为服务器动态改动脚本，
	//因为是事件触发，所以增加贩卖列表只能重启客户端才能看到.
	...
	if (player->AddItem(&item))
	{
		...
		// 		player->AddMoney(-item.GetItemBaseAttribute()->Money * count);
		// 		player->AddBijou(-item.GetItemBaseAttribute()->Bijou * count);
...
void CNPC::CalculatePassivePracticeDegree( int /*MagicID*/ , ATTACK_TYPE /*eAttackType*/,bool /*IsDead*/ )
{
	return ;
}

bool CNPC::CalculatePassivePracticeDegree(PhysiacalAttack /*Type*/, ATTACK_TYPE /*eAttackType*/, bool /*IsDead*/ )
{
	return false;
}

bool CNPC::ChanegeDegree(int /*ucSkillID*/, int /*Buffer*/)
{
	return false;
}

bool CNPC::PassiveSkillUpgrade(UINT /*SkillID*/, int *)
{
	return true;
}

void  CNPC::AddPassiveSkill(int /*ucSkillID*/, int /*iRank*/ , int /*EXP*/ )
{

}

INT CNPC::CalculateAttackDamage(  CGameObject& /*obj*/, float /*iPower*/  )
{
	return 0;
}

INT CNPC::CalculateAttackDamage(int /*MagicID*/)
{
	return 0;
}
